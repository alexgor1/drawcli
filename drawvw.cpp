...
void CDrawView::InvalObj(CDrawObj* pObj)
{
	...
	{
		rect.left -= kSnapMemoryDistance;
		rect.top -= kSnapMemoryDistance;
		rect.right += kSnapMemoryDistance;
		rect.bottom += kSnapMemoryDistance;
	}
...
BOOL CDrawView::OnIdle(LONG /*lCount*/)
{
	if (m_bActive) {
		CDrawTool* pTool = CDrawTool::FindTool(CDrawTool::c_drawShape);
		if (pTool != NULL)
