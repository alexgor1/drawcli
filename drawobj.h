...
// Attributes
	...
	virtual BOOL canSnap(int /*nHandleID*/) { return FALSE; }
	virtual void SetLastSnap(CPoint* point, int handle) { 
		m_lastSnap = point;
		m_snappedHandle = handle;
		Invalidate();
	}
// Operations
	virtual void Draw(CDC* pDC);
	virtual void DrawTracker(CDC* pDC, TrackerState state);
	virtual void DrawSnapping(CDC* pDC);
...
public:
	...
	virtual CPoint GetHandle(int nHandle);
	virtual HCURSOR GetHandleCursor(int nHandle);
	virtual BOOL canSnap(int nHandleID);
	virtual void MoveHandleTo(int nHandle, CPoint point, CDrawView* pView = NULL);
	virtual BOOL Intersects(const CRect& rect);
...
public:
	...
	virtual CPoint GetHandle(int nHandle);
	virtual HCURSOR GetHandleCursor(int nHandle);
	virtual BOOL canSnap(int /*nHandleID*/) { return TRUE; }
	virtual void MoveHandleTo(int nHandle, CPoint point, CDrawView* pView = NULL);
	virtual BOOL Intersects(const CRect& rect);
	virtual CDrawObj* Clone(CDrawDoc* pDoc);
